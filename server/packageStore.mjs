...
export function loadPackage(packagePath, options = {}) {
  try {
    ...
    // Plaintext is accepted on load without wipe. GET /api/package seals via getHydratedPackage.
...
/**
 * Atomic package save: serialize + validate, AES-256-GCM, backup previous good
 * ciphertext if none, write temp in the same directory, fsync, rename.
 *
 * Legacy plaintext is accepted on load without wipe. GET /api/package
 * (getHydratedPackage) seals in place atomically. Seal failure fails closed
 * and does not wipe the original file.
 *
 * options.crashAfterTempWrite is test-only: leave the temp file and skip rename.
 * options.audit === false skips the package-access audit line.
 */
